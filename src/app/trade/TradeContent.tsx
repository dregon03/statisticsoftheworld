...
interface Props {
  initialData: TradeData | null;
  initialCountries: string[];
}

export default function TradeContent({ initialData, initialCountries }: Props) {
  const [countryId, setCountryId] = useState(initialData?.countryId || 'USA');
  const [data, setData] = useState<TradeData | null>(initialData);
  const [loading, setLoading] = useState(false);
  const [countries, setCountries] = useState<string[]>(initialCountries);

  useEffect(() => {
    if (countryId === (initialData?.countryId || 'USA') && data === initialData) return;
    setLoading(true);
