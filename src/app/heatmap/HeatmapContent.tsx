...
export default function HeatmapContent({ initialData }: Props) {
  ...
  const [ranges, setRanges] = useState<Record<string, Range>>(initialData?.ranges || {});
  const [rows, setRows] = useState<CountryRow[]>(initialData?.countries || []);
  const [loading, setLoading] = useState(!initialData);
  const [presets, setPresets] = useState<{ id: string; label: string }[]>(initialData?.availablePresets || []);
  const [groups, setGroups] = useState<string[]>(initialData?.availableGroups || []);
  const [sortBy, setSortBy] = useState<string | null>(null);
  const [sortAsc, setSortAsc] = useState(false);
  const [hasFetchedOnce, setHasFetchedOnce] = useState(!!initialData);

  useEffect(() => {
    if (hasFetchedOnce && preset === 'macro' && group === 'Top20') return;
    setLoading(true);
  ...
  return (
    <main className="min-h-screen bg-[#f8f9fb] text-[#1a1a2e]">
      ...
      <section className="max-w-[1400px] mx-auto px-4 py-6">
          ...
          <div className="flex gap-1 bg-[#f0f0f0] rounded-lg p-0.5">
            {presets.map(p => (
              <button
                ...
                className={`px-3 py-1.5 rounded text-[14px] transition ${
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#f4f6f9] border-b border-[#d5dce6]">
                  <th className="px-3 py-2.5 text-left text-[15px] text-[#64748b] uppercase sticky left-0 bg-[#f4f6f9] z-10 min-w-[160px]">
                    Country
                  </th>
                  {indicators.map(ind => (
                    <th key={ind.id} className="px-2 py-2.5 text-center min-w-[90px]">
                      <button
                        ...
                        className={`text-[15px] uppercase transition ${
                ...
                {sorted.map(row => (
                  <tr key={row.countryId} className="border-b border-[#edf0f5] hover:bg-[#fafafa]">
                    ...
                    {indicators.map(ind => {
                      ...
                      if (!d) {
                        return <td key={ind.id} className="px-2 py-2 text-center text-[15px] text-[#94a3b8]">—</td>;
                      }
                      ...
                      return (
                        <td key={ind.id} className="px-1 py-1 text-center">
                          <div className={`rounded px-1.5 py-1 text-[15px] font-mono ${colorClass}`} title={`${d.value} (${d.year})`}>
        ...
        {/* Legend */}
        <div className="mt-4 flex items-center gap-3 text-[15px] text-[#64748b]">
