  ...
  return (
    <main className="min-h-screen bg-white text-[#333]">
      ...
      <section className="max-w-[1200px] mx-auto px-4 py-8">
        ...
        <p className="text-[13px] text-[#999] mb-6">Real-time pulse of the global economy.</p>
        ...
        {loading ? (
          <div className="text-center py-16 text-[#999]">Loading dashboard...</div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Markets */}
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              <div className="px-4 py-2.5 bg-[#f8f9fa] border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="divide-y divide-gray-50">
                {quotes.map(q => (
                  <div key={q.id} className="flex items-center justify-between px-4 py-2 text-[12px]">
                    <span className="text-[#333] font-medium truncate flex-1">{q.label}</span>
                    <span className="font-mono text-[#333] w-20 text-right">{q.price?.toFixed(2)}</span>
            ...
            <div className="border border-gray-100 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-gray-50">
                {events.length === 0 ? (
                  <div className="px-4 py-8 text-center text-[12px] text-[#999]">No upcoming events</div>
                ) : events.map((e, i) => (
                  <div key={i} className="px-4 py-2 text-[12px]">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[#333] font-medium truncate">{e.type === 'earnings' && e.symbol ? `${e.symbol} Earnings` : e.name}</span>
                    </div>
                    <div className="text-[11px] text-[#999] ml-3.5">{e.date} · {e.country}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="divide-y divide-gray-50">
                {trending.map((t, i) => (
                  <div key={i} className="px-4 py-2.5">
                    <Link href={`/indicators?id=${encodeURIComponent(t.indicatorId)}`} className="text-[13px] font-medium text-[#333] hover:text-[#0066cc]">
                      {t.title}
                    </Link>
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          {[
            ...
            <Link key={l.href} href={l.href} className="px-3 py-1.5 text-[12px] border border-gray-200 rounded-lg hover:bg-gray-50 text-[#666]">
              {l.label}
            </Link>
          ))}
        </div>
