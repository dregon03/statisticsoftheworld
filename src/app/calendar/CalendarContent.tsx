...
interface Props {
  ...
  initialMeta?: CalendarMeta;
}

export default function CalendarContent({ initialEvents, initialMeta }: Props) {
  const [events, setEvents] = useState<CalendarEvent[]>(initialEvents || []);
  const [meta, setMeta] = useState<CalendarMeta>(initialMeta || {});
  ...
  useEffect(() => {
    if (weekOffset === 0 && initialEvents) return;
    setLoading(true);
  ...
  }, [weekOffset, week.from, week.to, initialEvents]);
