...
interface RegionsProps {
  initialOverview?: Record<string, RegionResponse>;
  initialData?: RegionResponse;
}

export default function RegionsContent({ initialOverview, initialData }: RegionsProps) {
  return (
    <Suspense>
      <RegionsInner initialOverview={initialOverview} initialData={initialData} />
    </Suspense>
  );
}

function RegionsInner({ initialOverview, initialData }: RegionsProps) {
  const searchParams = useSearchParams();
  const urlIndicator = searchParams.get('id');

  const [selectedId, setSelectedId] = useState(urlIndicator || 'IMF.NGDPDPC');
  const [data, setData] = useState<RegionResponse | null>((!urlIndicator && initialData) ? initialData : null);
  const [loading, setLoading] = useState(!initialData || !!urlIndicator);
  const [view, setView] = useState<'region' | 'income'>('region');
  const [metric, setMetric] = useState<'avg' | 'median' | 'sum'>('avg');
  const [searchTerm, setSearchTerm] = useState('');

  const [overviewData, setOverviewData] = useState<Record<string, RegionResponse>>(initialOverview || {});
  ...
  // Fetch overview data on mount (skip if provided)
  useEffect(() => {
  ...
  // Fetch selected indicator data (skip initial if provided)
  useEffect(() => {
    if (!urlIndicator && initialData && selectedId === 'IMF.NGDPDPC') return;
    setLoading(true);
    fetch(`/api/regions?indicator=${encodeURIComponent(selectedId)}`)
      .then(r => r.json())
      .then(d => { setData(d); setLoading(false); })
      .catch(() => setLoading(false));
  }, [selectedId, urlIndicator, initialData]);
