...
interface Props {
  initialIndicators?: { id: string; label: string }[];
  initialYears?: number[];
  initialCountryData?: ForecastRow | null;
  initialCountryName?: string;
  initialIso2?: string;
}

export default function ForecastContent({ initialIndicators, initialYears, initialCountryData, initialCountryName, initialIso2 }: Props) {
  const params = useParams();
  const countryId = params.id as string;
  const [indicators, setIndicators] = useState<{ id: string; label: string }[]>(initialIndicators || []);
  const [selectedId, setSelectedId] = useState('IMF.NGDP_RPCH');
  const [years, setYears] = useState<number[]>(initialYears || []);
  const [countryData, setCountryData] = useState<ForecastRow | null>(initialCountryData || null);
  const [countryName, setCountryName] = useState(initialCountryName || '');
  const [iso2, setIso2] = useState(initialIso2 || '');
  const [loading, setLoading] = useState(!initialCountryData);

  useEffect(() => {
    if (initialCountryData && selectedId === 'IMF.NGDP_RPCH') return;
    setLoading(true);
  ...
  }, [selectedId, countryId, initialCountryData]);
