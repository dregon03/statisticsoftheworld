              ...
              <table className="w-full">
                ...
                <tbody>
                  {overviewData[OVERVIEW_INDICATORS[0].id]?.byRegion.map(region => (
                    <tr key={region.name} className="border-b border-[#f0f0f0] hover:bg-[#f8f9fa] transition">
          ...
          <div className="lg:col-span-3">
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="text-[12px] text-[#999]">{data?.totalCountries || 0} countries with data</div>
              </div>
              <div className="flex gap-2">
                <div className="flex gap-1 bg-[#f0f0f0] rounded-lg p-0.5">
                  <button
                    onClick={() => setView('region')}
                    className={`px-3 py-1 rounded text-[12px] transition ${view === 'region' ? 'bg-white shadow-sm font-medium' : 'text-[#666]'}`}
                  >
                    By Region
                  </button>
                  <button
                    onClick={() => setView('income')}
                    className={`px-3 py-1 rounded text-[12px] transition ${view === 'income' ? 'bg-white shadow-sm font-medium' : 'text-[#666]'}`}
                  >
                    By Income
                  </button>
                </div>
                <select
                  ...
                  className="border border-[#e8e8e8] rounded-lg px-2 py-1 text-[12px] outline-none"
                >
            ...
            {loading ? (
              <div className="text-center py-20 text-[#999]">Loading regional data...</div>
            ) : !currentData || currentData.length === 0 ? (
              <div className="text-center py-20 text-[#999]">No regional data available.</div>
            ) : (
              <>
                {/* Bar visualization */}
                  ...
                  {currentData.map(region => {
                    ...
                    return (
                      <div key={region.name} className="flex items-center gap-3">
                        <div className="w-[180px] text-[13px] font-medium text-[#333] shrink-0 truncate">{region.name}</div>
                        <div className="flex-1 flex items-center gap-2">
                          ...
                          <div className="w-[100px] text-right font-mono text-[12px] text-[#333] shrink-0">
                            {formatValue(region[metric], selectedIndicator?.format || 'number', selectedIndicator?.decimals)}
                          </div>
                        </div>
                        <div className="w-[40px] text-right text-[11px] text-[#999] shrink-0">
                          n={region.countries}
                        </div>
                      </div>
                    );
                    ...
                    <tbody>
                      {currentData.map(region => (
                        <tr key={region.name} className="border-b border-[#f0f0f0] hover:bg-[#f8f9fa] transition">
                          ...
                          <td className="px-4 py-2.5 text-right text-[12px] text-[#999]">{region.countries}</td>
                          ...
                          <td className="px-4 py-2.5 text-right font-mono text-[12px] text-[#999]">
