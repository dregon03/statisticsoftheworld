...
export default function IndicatorExplorer() {
  return (
    <Suspense>
      <IndicatorExplorerContent />
    </Suspense>
  );
}

function IndicatorExplorerContent() {
  ...
  return <ExplorerView initialId={initialId} />;
}

function ExplorerView({ initialId }: { initialId: string }) {
  const initialIndicator = INDICATORS.find(i => i.id === initialId) || INDICATORS[0];
  ...
  const [sortBySource, setSortBySource] = useState<string | null>(null);
          ...
          {loading ? (
            ...
            (() => {
              ...
              if (hasMultiSource && multiData && multiData.countries.length > 0) {
                ...
                chartData = multiData.countries
                  ...
                  .map(c => ({
                    country: c.country, countryId: c.countryId, iso2: c.iso2,
                    value: c.values[primaryId].value as number, year: c.values[primaryId].year,
                  }))
              ...
              return top30.length > 0 ? (
                <div className="border border-gray-100 rounded-xl p-4">
                  <BarRankingChart data={top30} format={selectedIndicator.format} decimals={selectedIndicator.decimals} maxItems={30} label={`Top ${Math.min(30, chartData.length)} \u2014 ${selectedIndicator.label}`} />
                </div>
              ) : (
            ...
            (() => {
              ...
              const sorted = [...multiData.countries].sort((a, b) => {
                return sortAsc ? getBestValue(a) - getBestValue(b) : getBestValue(b) - getBestValue(a);
              });
              return (
                    ...
                    <thead>
                      <tr className="text-left text-xs border-b border-gray-100 bg-gray-50">
                        <th className="px-4 py-2.5 w-10">
                          <button onClick={() => setSortAsc(!sortAsc)} className="text-gray-400 hover:text-gray-700 transition">
                            {sortAsc ? '#\u2191' : '#\u2193'}
                          </button>
                        </th>
                        ...
                        {multiData.sources.map(src => {
                          ...
                          return (
                            <th key={src.id} className="px-4 py-2.5 text-right">
                              <button onClick={() => setSortBySource(src.id)} className={`font-semibold transition ${isActive ? 'text-blue-600' : 'text-gray-400 hover:text-gray-700'}`}>
                                {src.org}{isActive ? ' \u25BC' : ''}
                              </button>
                            </th>
                          );
                    ...
                    <tbody>
                      {sorted.map((row, i) => (
                        <tr key={row.countryId} className="border-b border-gray-50 hover:bg-gray-50 transition">
                          <td className="px-4 py-2 text-gray-300 text-sm">{i + 1}</td>
                          ...
                          {multiData.sources.map(src => {
                            ...
                            return (
                              <td key={src.id} className="px-4 py-2 text-right font-mono text-sm">
                                {d ? (<>{formatValue(d.value, selectedIndicator.format, selectedIndicator.decimals)}<span className="text-gray-300 text-xs ml-1">{d.year}</span></>) : <span className="text-gray-300">&mdash;</span>}
                              </td>
                            );
                          })}
                        </tr>
            ...
            (() => {
              const sourceName = selectedIndicator.id.startsWith('IMF.') ? 'IMF' : selectedIndicator.id.startsWith('UN.') ? 'United Nations' : 'World Bank';
              return (
