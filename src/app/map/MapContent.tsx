...
export default function MapContent({ initialData }: MapProps) {
  return (
    <main className="min-h-screen bg-[#f8f9fb] text-[#1a1a2e]">
      <Nav />
      <HeroTabs active="/map" />
      <Suspense>
        <MapInner initialData={initialData} />
      </Suspense>
      <Footer />
    </main>
  );
...
export function MapInner({ initialData }: { initialData?: RankingEntry[] }) {
  ...
  const [data, setData] = useState<RankingEntry[]>((!initialId && initialData) ? initialData : []);
  const [loading, setLoading] = useState(!initialData || !!initialId);
  const [searchTerm, setSearchTerm] = useState('');

  useEffect(() => {
    if (!initialId && initialData && selectedIndicator.id === MAP_INDICATORS[0].id) return;
    setLoading(true);
  ...
  }, [selectedIndicator, initialId, initialData]);
