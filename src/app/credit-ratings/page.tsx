        ...
        <div className="flex flex-wrap gap-3 mb-6">
          ...
          <div className="flex gap-1 ml-auto bg-[#f0f0f0] rounded-lg p-0.5">
            <button
              ...
              className={`px-3 py-1 rounded text-[12px] transition ${viewMode === 'table' ? 'bg-white shadow-sm font-medium' : 'text-[#666]'}`}
            >Table</button>
            <button
              onClick={() => setViewMode('map')}
              className={`px-3 py-1 rounded text-[12px] transition ${viewMode === 'map' ? 'bg-white shadow-sm font-medium' : 'text-[#666]'}`}
            >Map</button>
          </div>
        </div>

        {loading ? (
          <div className="text-center py-20 text-[#999]">Computing credit scores...</div>
        ) : viewMode === 'map' ? (
          <div>
            <WorldMap
              data={mapData}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                ...
                {sorted.slice(0, 10).map((r, i) => (
                  <div key={r.countryId} className="flex items-center justify-between py-1 text-[12px]">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-[#333] w-8 text-right">{r.score}</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="border border-[#e8e8e8] rounded-xl p-4">
                ...
                {[...sorted].reverse().slice(0, 10).map((r, i) => (
                  <div key={r.countryId} className="flex items-center justify-between py-1 text-[12px]">
                    <div className="flex items-center gap-2">
                      <span className="text-[#999] w-4">{sorted.length - i}.</span>
                      <Flag iso2={r.iso2} size={16} />
                      <Link href={`/country/${r.countryId}`} className="text-[#0066cc] hover:underline">{r.country}</Link>
                    </div>
                    <div className="flex items-center gap-2">
          ...
          <div className="border border-[#e8e8e8] rounded-xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="text-[11px] text-[#999] uppercase border-b border-[#e8e8e8] bg-[#f8f9fa]">
                  ...
                  <th className="px-4 py-2.5 text-center">
                    <button onClick={() => { setSortBy('grade'); setSortAsc(!sortAsc); }} className="hover:text-[#333] transition">
                      SOTW Grade
                    </button>
                  </th>
                  <th className="px-4 py-2.5 text-right">
                    <button onClick={() => { setSortBy('score'); setSortAsc(!sortAsc); }} className="hover:text-[#333] transition">
                      Score (0-100)
                    </button>
                  </th>
              ...
              <tbody>
                {sorted.map((r, i) => (
                  <tr key={r.countryId} className="border-b border-[#f0f0f0] hover:bg-[#f8f9fa] transition">
                    <td className="px-4 py-2 text-[12px] text-[#999]">{i + 1}</td>
                    ...
                    <td className="px-4 py-2 text-right font-mono text-[12px] text-[#666] hidden md:table-cell">{r.governance}</td>
                    <td className="px-4 py-2 text-right font-mono text-[12px] text-[#666] hidden md:table-cell">
                      {r.gdpPerCapita ? `$${r.gdpPerCapita.toLocaleString()}` : '—'}
                    </td>
                    <td className="px-4 py-2 text-right font-mono text-[12px] text-[#666] hidden md:table-cell">
                      {r.debtToGdp ? `${r.debtToGdp}%` : '—'}
                    </td>
                    <td className="px-4 py-2 text-[11px] text-[#999] hidden lg:table-cell">{r.region}</td>
                  </tr>
                ))}
              </tbody>
            </table>
