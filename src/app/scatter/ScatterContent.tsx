...
interface ScatterProps {
  ...
  initialXData?: IndicatorData[];
  initialYData?: IndicatorData[];
  initialPopData?: IndicatorData[];
  initialCorrelations?: { x: string; y: string; labelX: string; labelY: string; r: number; rSquared: number; n: number }[];
}

export default function ScatterContent(props: ScatterProps) {
  return (
    <Suspense>
      <ScatterInner {...props} />
    </Suspense>
...
function ScatterInner({ initialCountries, initialXData, initialYData, initialPopData, initialCorrelations }: ScatterProps) {
  ...
  const hasInitial = !!(initialCountries && initialXData && initialYData);
  const [countries, setCountries] = useState<Country[]>(initialCountries || []);
  const [xData, setXData] = useState<IndicatorData[]>(initialXData || []);
  const [yData, setYData] = useState<IndicatorData[]>(initialYData || []);
  const [popData, setPopData] = useState<IndicatorData[]>(initialPopData || []);
  ...
  const [correlations, setCorrelations] = useState<{ x: string; y: string; labelX: string; labelY: string; r: number; rSquared: number; n: number }[]>(initialCorrelations || []);
  const [corrFilter, setCorrFilter] = useState<'strongest' | 'surprising' | 'negative'>('strongest');

  // Fetch precomputed correlations
  useEffect(() => {
    if (initialCorrelations) return;
  ...
  useEffect(() => {
    if (hasInitial && xIndicatorId === defaultPreset.x && yIndicatorId === defaultPreset.y) return;
    setLoading(true);
    Promise.all([
  ...
  }, [xIndicatorId, yIndicatorId, hasInitial, defaultPreset.x, defaultPreset.y]);
