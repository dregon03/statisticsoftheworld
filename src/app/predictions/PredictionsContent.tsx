...
interface Props {
  ...
  initialByCategory?: Record<string, PredictionMarket[]>;
  initialUpdatedAt?: string | null;
}

export default function PredictionsContent({ initialMarkets, initialByCategory, initialUpdatedAt }: Props) {
  const [markets, setMarkets] = useState<PredictionMarket[]>(initialMarkets || []);
  const [byCategory, setByCategory] = useState<Record<string, PredictionMarket[]>>(initialByCategory || {});
  ...
  useEffect(() => {
    if (!initialMarkets) fetchPredictions();
    const iv = setInterval(() => fetchPredictions(true), 60_000);
    return () => clearInterval(iv);
  }, [fetchPredictions, initialMarkets]);
